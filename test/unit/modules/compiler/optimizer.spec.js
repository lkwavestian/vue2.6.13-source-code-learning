...
describe("optimizer", () => {
  ...
  it("mark static trees inside v-for with nested v-else and v-once", () => {
    ...
    optimize(ast, baseOptions);
    expect(ast.ifConditions[1].block.children[0].children[0].ifConditions[1].block.staticRoot).toBe(
      false
    );
    expect(
      ast.ifConditions[1].block.children[0].children[0].ifConditions[1].block.staticInFor
    ).toBe(true);

    expect(ast.ifConditions[1].block.children[0].children[0].ifConditions[2].block.staticRoot).toBe(
      false
    );
    expect(
      ast.ifConditions[1].block.children[0].children[0].ifConditions[2].block.staticInFor
    ).toBe(true);

    expect(ast.ifConditions[2].block.children[0].children[0].ifConditions[1].block.staticRoot).toBe(
      false
    );
    expect(
      ast.ifConditions[2].block.children[0].children[0].ifConditions[1].block.staticInFor
    ).toBe(true);
  });
});
